 ...
 *
 * Phase 6 fix: MicController is the single source of mic ownership.
 *   Owner tag = MicController.OWNER_WAKE.
 */
class LiveKitWakeWordEngine(
     ...
     * Phase 6: Acquires mic via MicController.OWNER_WAKE exclusively.
     */
    fun startMonitoring(): Boolean {
        ...
        if (!micController.acquireMic(MicController.OWNER_WAKE)) {
            val holder = micController.getCurrentOwner()
            val msg = "Microphone busy${holder?.let { " (held by $it)" } ?: ""} — cannot start wake monitoring"
            Log.w(TAG, msg)
            onErrorCallback?.invoke(IllegalStateException(msg))
            return false
        }
     ...
     * Phase 6: Only acquires mic if MicController grants ownership.
     */
    fun resume() {
        if (!isMonitoring || !pausedForCommand) return
        if (!micController.acquireMic(MicController.OWNER_WAKE)) {
            val holder = micController.getCurrentOwner()
            Log.w(TAG, "Wake-word resume deferred — mic busy${holder?.let { " (held by $it)" } ?: ""}")
            return
        }
        pausedForCommand = false
        detector.resume()
        if (detector.isAvailable()) {
            ...
            // Phase 1: no STT fallback on resume either.
            Log.w(TAG, "Cannot resume wake-word: ONNX unavailable. Releasing mic.")
            micController.releaseMic(MicController.OWNER_WAKE)
            isMonitoring = false
