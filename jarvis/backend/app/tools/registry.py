...
class ToolDefinition(BaseModel):
    ...
    capability_tier: str = "safe"  # safe, device_control, automation, high_risk
...
class ToolRegistry:
    ...
    def _register_default_tools(self) -> None:
        ...
        self.register(ToolDefinition(
            ...
            capability_tier="device_control",
        ...
        self.register(ToolDefinition(
            ...
            capability_tier="safe", platform="backend", parameters={"query": ToolParameter(type="string", description="Search query string")}
        ))
        self.register(ToolDefinition(
            name="analyze_image", description="Analyze image or screenshot contents using multimodal vision model.", risk_level="safe",
            capability_tier="safe", platform="backend", parameters={"image_url_or_base64": ToolParameter(type="string", description="Base64 image data or URL"), "prompt": ToolParameter(type="string", description="Question about the image", required=False)}
        ))
        self.register(ToolDefinition(
            name="search_music",
            description=(
            ...
            risk_level="safe", capability_tier="safe", platform="backend",
            parameters={
        ...
        self.register(ToolDefinition(
            ...
            description="Control smart-home devices via Home Assistant / Matter bridge (lights, switches, climate/AC, fans, locks, scenes).",
            risk_level="safe",
            capability_tier="device_control",
            platform="backend",
            parameters={
                "action": ToolParameter(type="string", description="Action: turn_on, turn_off, toggle, set_level, set_temperature, get_status", enum=["turn_on", "turn_off", "toggle", "set_level", "set_temperature", "get_status"]),
                "entity": ToolParameter(type="string", description="Device or entity name, e.g. living_room_light, ac, fan, bedroom_lamp"),
                "value": ToolParameter(type="string", description="Optional level or temperature value, e.g. '24' or '75'", required=False),
                "area": ToolParameter(type="string", description="Optional room or area name, e.g. living room, bedroom, kitchen", required=False),
            }
        ))
